...
final class ScanStore: ObservableObject {
    ...
    init() {
        plans = (try? storage.load()) ?? []
    }
    
    func save(plan: FloorPlan) {
        var p = plan
        p.rooms = p.rooms.map { var r = $0; var x = r; x.computeArea(); return x }
        plans.append(p)
        try? storage.save(plans)
    }
...
struct Storage {
    ...
    func save(_ plans: [FloorPlan]) throws {
        let data = try JSONEncoder().encode(plans)
        try data.write(to: url)
    }
    func load() throws -> [FloorPlan] {
