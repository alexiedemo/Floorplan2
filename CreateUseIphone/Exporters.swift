...
struct ExportView: View {
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            VStack(spacing: 8) {
                Button { exportURL = SVGExporter().export(plan: plan); showingShare = true } label: { Label("Export SVG", systemImage: "square.and.arrow.down") }
                Button { exportURL = PDFExporter().export(plan: plan); showingShare = true } label: { Label("Export PDF", systemImage: "doc.richtext") }
                Button { exportURL = JSONExporter().export(plan: plan); showingShare = true } label: { Label("Export JSON", systemImage: "curlybraces") }
                Button { exportURL = CSVExporter().export(plan: plan); showingShare = true } label: { Label("Export CSV", systemImage: "tablecells") }
            }
        }
        .padding()
        .sheet(isPresented: $showingShare) { if let url = exportURL { ShareSheet(items: [url]) } }
    }
}
...
struct JSONExporter { func export(plan: FloorPlan) -> URL? {
...
struct SVGExporter {
    func export(plan: FloorPlan) -> URL? {
        let url = FileManager.default.temporaryDirectory.appendingPathComponent("floorplan.svg")
        let svg = SVGRenderer().render(plan: plan, size: CGSize(width: 1200, height: 800))
        try? svg.data(using: .utf8)?.write(to: url); return url
    }
}
...
final class FloorPlanPDFView: UIView {
    let plan: FloorPlan; init(plan: FloorPlan){ self.plan = plan; super.init(frame: .zero); backgroundColor = .white }
    required init?(coder: NSCoder) { fatalError() }
    override func draw(_ rect: CGRect) {
        let padding: CGFloat = 40, maxW = rect.width - 2*padding, maxH = rect.height - 2*padding
        let all = plan.rooms.flatMap { $0.corners }; guard !all.isEmpty else { return }
        ...
        for room in plan.rooms {
            ...
            ctx.beginPath(); ctx.move(to: first); for p in pts.dropSecondOrSelf() { ctx.addLine(to: p) }; ctx.closePath()
...
extension Array {
    func dropSecondOrSelf() -> ArraySlice<Element> {
...
struct SVGRenderer {
    func render(plan: FloorPlan, size: CGSize) -> String {
        ...
        var shapes:[String]=[]
        for room in plan.rooms {
            let points = room.corners.map { p -> String in let (x,y)=pt(p); return "\(Int(x)),\(Int(y))" }.joined(separator: " ")
