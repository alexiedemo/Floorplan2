...
func drawPlan(_ plan: FloorPlan, in ctx: GraphicsContext, size: CGSize) {
    ...
    let scale = min((size.width-2*padding)/max(w,0.1), (size.height-2*padding)/max(h,0.1))
    func pt(_ v: V2) -> CGPoint { CGPoint(x: (v.x - minX)*scale + padding, y: (v.y - minY)*scale + padding) }
    for room in plan.rooms {
        var path = Path()
        let pts = room.corners.map { pt($0) }
        guard let first = pts.first else { continue }
        path.move(to: first); for p in pts.dropFirst() { path.addLine(to: p) }; path.closeSubpath()
        ctx.stroke(path, with: .color(.black), lineWidth: 2)
        ctx.fill(path, with: .color(.blue.opacity(0.06)))
    }
}
